...
local config_module = require("herdr-watch.config")
    ...
    if not defer_socket_sync then
      ...
      for _, event in ipairs(buffered) do
        ...
        if (event.event == "pane_moved" or event.event == "pane.moved") and event.data and event.data.pane then
          ...
          if migrate_err then
            vim.notify("herdr-watch: " .. migrate_err, vim.log.levels.WARN)
          end
        end
      end
    end
...
local function connect_status_client(pane_id)
  ...
  local request_id = "herdr-watch:status:" .. tostring(request_number)
  local expected_socket_generation = socket_generation
  local status_client
...
connect_socket = function(restarting)
  ...
  start_polling(false)
  request_number = request_number + 1
  local request_id = "herdr-watch:" .. tostring(request_number)
...
function M.start(options, opts)
  ...
  augroup = vim.api.nvim_create_augroup("HerdrWatchPresence", { clear = true })

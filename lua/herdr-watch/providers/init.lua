...
local builtin_modules = {
  "herdr-watch.providers.selection",
  "herdr-watch.providers.symbol",
  "herdr-watch.providers.hunk",
  "herdr-watch.providers.diagnostics",
  "herdr-watch.providers.quickfix",
  "herdr-watch.providers.trouble",
}

local function validate_provider(provider)
  if type(provider) ~= "table" then
    error("herdr-watch: provider must be a table")
  end
  vim.validate({
  ...
  if provider.id == "" or not provider.id:match("^[%w_.-]+$") then
    error("herdr-watch: provider.id must contain only letters, numbers, dots, underscores, and dashes")
  end
  if provider.name == "" then
    error("herdr-watch: provider.name must not be empty")
  end
  if provider.priority ~= nil and type(provider.priority) ~= "number" then
    error("herdr-watch: provider.priority must be a number")
  end
  if provider.available ~= nil and type(provider.available) ~= "function" then
    error("herdr-watch: provider.available must be a function")
  end
end
...
function M.register(provider, opts)
  ...
  if registry[provider.id] and not opts.replace then
    error("herdr-watch: provider " .. provider.id .. " is already registered")
  end

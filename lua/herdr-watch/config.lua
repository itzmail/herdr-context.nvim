...
local function validate_choice(name, value, choices)
  if not vim.tbl_contains(choices, value) then
    error(("herdr-watch: %s must be one of: %s"):format(name, table.concat(choices, ", ")))
  end
end
...
local function validate(opts)
  ...
  if opts.max_payload_bytes <= 0 or opts.max_payload_bytes % 1 ~= 0 then
    error("herdr-watch: max_payload_bytes must be a positive integer")
  end
  ...
  for _, key in ipairs({ "width", "height" }) do
    ...
    if value <= 0 then
      error("herdr-watch: composer." .. key .. " must be greater than zero")
    end
  end
  if opts.composer.checklist_width <= 0 or opts.composer.checklist_width >= 1 then
    error("herdr-watch: composer.checklist_width must be between zero and one")
  end
  if opts.composer.provider_timeout_ms <= 0 or opts.composer.provider_timeout_ms % 1 ~= 0 then
    error("herdr-watch: composer.provider_timeout_ms must be a positive integer")
  end
  if opts.composer.hunk_context_lines < 0 or opts.composer.hunk_context_lines % 1 ~= 0 then
    error("herdr-watch: composer.hunk_context_lines must be a non-negative integer")
  end
  for name, value in pairs(opts.composer.defaults) do
    if type(value) ~= "boolean" then
      error("herdr-watch: composer.defaults." .. tostring(name) .. " must be a boolean")
    end
  end
  for name, preset in pairs(opts.composer.presets) do
    if type(name) ~= "string" or name == "" or type(preset) ~= "table" then
      error("herdr-watch: composer.presets must map non-empty names to provider lists")
    end
    for _, id in ipairs(preset) do
      if type(id) ~= "string" or id == "" then
        error("herdr-watch: composer preset entries must be non-empty provider ids")
      end
    end
  end
  for _, key in ipairs({ "exclude_patterns", "secret_patterns" }) do
    for _, pattern in ipairs(opts.safety[key]) do
      if type(pattern) ~= "string" or pattern == "" then
        error("herdr-watch: safety." .. key .. " entries must be non-empty strings")
      end
    end
  end
  if opts.history.max_entries <= 0 or opts.history.max_entries % 1 ~= 0 then
    error("herdr-watch: history.max_entries must be a positive integer")
  end
  ...
  for _, mode in ipairs(opts.providers.trouble.modes) do
    if type(mode) ~= "string" or mode == "" then
      error("herdr-watch: providers.trouble.modes entries must be non-empty strings")
    end
  end
  ...
  if opts.agents_view.preview_lines <= 0 or opts.agents_view.preview_lines % 1 ~= 0 then
    error("herdr-watch: agents_view.preview_lines must be a positive integer")
  end
  if opts.agents_view.deep_preview_lines <= 0 or opts.agents_view.deep_preview_lines % 1 ~= 0 then
    error("herdr-watch: agents_view.deep_preview_lines must be a positive integer")
  end
  if opts.agents_view.preview_width < 20 or opts.agents_view.preview_width % 1 ~= 0 then
    error("herdr-watch: agents_view.preview_width must be an integer of at least 20")
  end
  for name, icon in pairs(opts.statusline.icons) do
    if type(icon) ~= "string" then
      error("herdr-watch: statusline.icons." .. tostring(name) .. " must be a string")
    end
  end
end

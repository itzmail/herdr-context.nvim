  ...
  for callback in pairs(subscribers) do
    ...
    if not ok then
      vim.schedule(function()
        vim.notify("herdr-watch state subscriber failed: " .. tostring(err), vim.log.levels.ERROR)
      end)
    end
  end
  events.emit("HerdrWatchUpdated", public)
...
local function emit_transitions(previous, current)
  if previous.connected ~= current.connected then
    events.emit(current.connected and "HerdrWatchConnected" or "HerdrWatchDisconnected", {
      mode = current.mode,
      stale = current.stale,
    })
  end
  ...
  for pane_id, agent in pairs(current.agents_by_pane) do
    ...
    if old and old.agent_status ~= agent.agent_status then
      events.emit("HerdrWatchAgentStatusChanged", {
        pane_id = pane_id,
...
function M.set_target(pane_id)
  ...
  events.emit("HerdrWatchTargetChanged", {
    pane_id = pane_id,

...
local namespace = vim.api.nvim_create_namespace("herdr-watch-agents")
local bufnr
local winid
local line_to_pane = {}
local line_to_group = {}
...
local status_highlights = {
  idle = "HerdrWatchIdle",
  working = "HerdrWatchWorking",
  blocked = "HerdrWatchBlocked",
  done = "HerdrWatchDone",
  unknown = "HerdrWatchUnknown",
}
...
local function setup_highlights()
  for name, link in pairs({
    HerdrWatchIdle = "DiagnosticOk",
    HerdrWatchDone = "DiagnosticOk",
    HerdrWatchWorking = "DiagnosticInfo",
    HerdrWatchBlocked = "DiagnosticError",
    HerdrWatchUnknown = "Comment",
    HerdrWatchTarget = "Special",
    HerdrWatchDisconnected = "DiagnosticWarn",
  }) do
...
function M.preview(opts)
  ...
  local cfg = config.get()
  require("herdr-watch.ui.preview").open(agent, {
...
function M.explain()
  local agent = selected_agent()
  if agent then
    require("herdr-watch.ui.explain").open(agent, { source_winid = winid })
  end
end
...
local function cleanup()
  require("herdr-watch.ui.preview").close()
  require("herdr-watch.ui.explain").close()
  if subscriber then
...
function M.open()
  ...
  vim.bo[bufnr].filetype = "herdr-watch-agents"
  vim.bo[bufnr].modifiable = false

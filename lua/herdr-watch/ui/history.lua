...
function M.open()
  ...
  vim.bo[bufnr].filetype = "herdr-watch-history"
    ...
    targets.resolve(config.get(), picker, {}, function(target, err)
      if not target then
        if err ~= "Target selection cancelled" then
          vim.notify(err, vim.log.levels.ERROR, { title = "herdr-watch.nvim" })
        end
        return
      end
      transport.stage(config.get(), target, entry.payload, function(ok, stage_err, result)
        if not ok then
          vim.notify(stage_err, vim.log.levels.ERROR, { title = "herdr-watch.nvim" })
          return
        end

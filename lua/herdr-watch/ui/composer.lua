...
local namespace = vim.api.nvim_create_namespace("herdr-watch-composer")
local active
...
local function setup_highlights()
  for name, link in pairs({
    HerdrWatchComposerHeader = "Title",
    HerdrWatchComposerStale = "DiagnosticWarn",
    HerdrWatchComposerError = "DiagnosticError",
    HerdrWatchComposerSize = "Comment",
    HerdrWatchComposerFooter = "Comment",
    HerdrWatchComposerLabel = "Comment",
    HerdrWatchComposerInstruction = "Special",
  }) do
...
local function render_list(ui)
  ...
  local marks = {
    { line = 0, start_col = 0, end_col = #lines[1], hl = "HerdrWatchComposerHeader" },
    { line = 2, start_col = 2, end_col = 8, hl = "HerdrWatchComposerLabel" },
    { line = 3, start_col = 2, end_col = 8, hl = "HerdrWatchComposerLabel" },
    { line = 4, start_col = 2, end_col = #lines[5], hl = "HerdrWatchComposerInstruction" },
    { line = 5, start_col = 2, end_col = 8, hl = "HerdrWatchComposerLabel" },
    { line = 6, start_col = 2, end_col = 8, hl = "HerdrWatchComposerLabel" },
  }
  ui.line_to_id = {}
  ...
  for _, entry in ipairs(session.entries) do
    ...
    if summary ~= "" then
      ...
      marks[#marks + 1] = {
        ...
        start_col = 5,
        end_col = #lines[#lines],
        hl = entry.excluded and "HerdrWatchComposerError" or "Comment",
      }
    end
  end
  ...
  for index = #lines - 2, #lines do
    marks[#marks + 1] = { line = index - 1, start_col = 0, end_col = #lines[index], hl = "HerdrWatchComposerFooter" }
  end
...
local function cleanup(ui)
  ...
  require("herdr-watch.ui.instruction").close(ui.session)
...
local function edit_instruction(ui)
  require("herdr-watch.ui.instruction").open(ui.session)
end
...
function M.open(session)
  ...
  configure_buffer(list_bufnr, "herdr-watch-composer")
  configure_buffer(preview_bufnr, "herdr-watch-preview")
  ...
  for _, bufnr in ipairs({ list_bufnr, preview_bufnr }) do
    ...
    map(bufnr, "h", function()
      require("herdr-watch.ui.history").open()

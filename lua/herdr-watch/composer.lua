...
local providers = require("herdr-watch.providers")
...
function M.capture_request(opts)
  ...
  if not vim.api.nvim_buf_is_valid(bufnr) then
    error("herdr-watch: the source buffer is no longer valid")
  end
...
local function rescope_diagnostics(session)
  ...
  diagnostic.section = require("herdr-watch.providers.diagnostics").section_for_range(session.request, range)
end
    ...
    targets.resolve(config.get(), picker, {}, function(target, target_err)
      ...
      transport.stage(config.get(), target, self.bundle.payload, function(ok, err, result)
        ...
        if preview_result then
          vim.schedule(function()
            require("herdr-watch.ui.preview").open(result.agent or target)
          end)
        end
      end, {
...
function M.open(opts)
  ...
  require("herdr-watch.ui.composer").open(session)
  collect(session)
  if opts.edit_instruction then
    vim.schedule(function()
      if not session.closed then
        require("herdr-watch.ui.instruction").open(session)
      end
    end)
  end
  return session

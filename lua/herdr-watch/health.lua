...
local function check_companion(cfg)
  local output, err = herdr.run(cfg, { "plugin", "list", "--plugin", "herdr-watch", "--json" })
  if not output then
    health.warn("Could not inspect the Herdr companion plugin: " .. err)
    return
  end
  if output:find('"herdr%-watch"') or output:find('"id"%s*:%s*"herdr%-watch"') then
    ...
    health.warn("Herdr companion plugin is not installed; run `herdr plugin install itzmail/herdr-watch.nvim`")
  end
end

local function check_providers(cfg)
  local count = #require("herdr-watch.providers").list()

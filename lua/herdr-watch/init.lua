...
local picker = require("herdr-watch.picker")
  ...
  safety.confirm(warnings, function(confirmed)
    ...
    targets.resolve(cfg, picker, {}, function(target, target_err)
      ...
      transport.stage(cfg, target, payload, function(ok, transport_err, result)
        ...
        require("herdr-watch.history").record({
          kind = kind,
          target = target,
          payload = payload,
          bytes = #payload,
...
function M.setup(opts)
  ...
  require("herdr-watch.ui.statusline").setup()
  require("herdr-watch.notifications").setup()
  watch.start(cfg)
  return cfg
end
...
function M.compose(opts)
  return require("herdr-watch.composer").open(opts)
end
...
function M.prompt(opts)
  ...
  if opts.timeout_ms and (opts.timeout_ms <= 0 or opts.timeout_ms % 1 ~= 0) then
    error("herdr-watch: prompt timeout_ms must be a positive integer")
  end
  if opts.timeout_ms and not opts.wait then
    error("herdr-watch: prompt timeout_ms requires wait = true")
  end
...
function M.delegate(opts)
  ...
  if opts.kind == "" then
    error("herdr-watch: delegate kind must not be empty")
  end
  if opts.placement and not vim.tbl_contains({ "split", "tab", "workspace" }, opts.placement) then
    error("herdr-watch: delegate placement must be split, tab, or workspace")
  end
  if opts.direction and not vim.tbl_contains({ "right", "down" }, opts.direction) then
    error("herdr-watch: delegate direction must be right or down")
  end
  if opts.timeout_ms and (opts.timeout_ms <= 0 or opts.timeout_ms % 1 ~= 0) then
    error("herdr-watch: delegate timeout_ms must be a positive integer")
  end
  if opts.timeout_ms and opts.wait == false then
    error("herdr-watch: delegate timeout_ms requires wait = true or an interactive tracking choice")
  end
  if
    opts.startup_timeout_ms
    and (opts.startup_timeout_ms <= 3000 or opts.startup_timeout_ms > 300000 or opts.startup_timeout_ms % 1 ~= 0)
  then
    error("herdr-watch: delegate startup_timeout_ms must be greater than 3000 and at most 300000")
  end
  if opts.agent_args and not vim.islist(opts.agent_args) then
    error("herdr-watch: delegate agent_args must be a list")
  end
  for _, arg in ipairs(opts.agent_args or {}) do
    if type(arg) ~= "string" then
      error("herdr-watch: delegate agent_args entries must be strings")
    end
  end
  return require("herdr-watch.delegate").open(opts)
end

function M.symbol(opts)
  return require("herdr-watch.composer").stage_provider("symbol", opts)
end

function M.hunk(opts)
  return require("herdr-watch.composer").stage_provider("hunk", opts)
end

function M.quickfix(opts)
  return require("herdr-watch.composer").stage_provider("quickfix", opts)
end

function M.location_list(opts)
  return require("herdr-watch.composer").stage_provider("location_list", opts)
end

function M.register_provider(provider)
  return require("herdr-watch.providers").register(provider)
end
...
function M.statusline()
  return require("herdr-watch.ui.statusline").get()
end

function M.agents()
  require("herdr-watch.ui.agents").toggle()
end

function M.explain_agent(target)
  local function open(agent)
    require("herdr-watch.ui.explain").open(agent)
  end
  if target then
    open(target)
    return
